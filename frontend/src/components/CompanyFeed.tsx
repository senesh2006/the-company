...
import { 
  Send, ShieldCheck, CheckCircle2, XCircle, RotateCcw, AlertTriangle, 
  Clock, Cpu, Sparkles, Filter, ChevronRight, Terminal, Layers
} from "lucide-react";
...
export function CompanyFeed() {
  ...
  const getTierColor = (tier?: TrustTier) => {
    if (tier === "operate") return "bg-emerald-500/10 text-emerald-400 border-emerald-500/30";
    if (tier === "assist") return "bg-blue-500/10 text-blue-400 border-blue-500/30";
  ...
  return (
    <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl shadow-2xl space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold text-white">Company Feed</h2>
              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 uppercase font-semibold">
                Live Audit Stream
              </span>
            </div>
      ...
      {activeTab === "dispatch" && (
        <form onSubmit={handleDispatch} className="p-5 rounded-2xl bg-slate-950/70 border border-cyan-500/20 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 font-mono resize-none"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {/* Assignee / Role */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Target Specialist / Router</label>
              <select
                value={selectedRole}
                onChange={(e) => setSelectedRole(e.target.value)}
                className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-cyan-500/50"
              >
            ...
            {/* Cadence */}
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Execution Cadence</label>
              <select
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-bold shadow-lg shadow-cyan-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isDispatching ? (
              <>
      ...
      {activeTab === "feed" && (
        <div className="space-y-3">
          {isLoading ? (
            ...
            <div className="p-10 rounded-2xl bg-white/[0.02] border border-white/5 text-center text-slate-400">
              <Clock className="w-8 h-8 mx-auto mb-2 text-slate-500" />
              <p className="text-sm font-semibold text-slate-300">No feed items recorded yet</p>
            ...
            feedItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="p-4 rounded-2xl bg-slate-950/40 border border-white/5 hover:border-white/10 transition-all space-y-3"
                >
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-white">{item.agent_name || item.role || "Autonomous Worker"}</span>
                  ...
                  <div>
                    ...
                    {item.mandate && (
                      <p className="text-[11px] text-slate-400 font-mono mt-1 bg-white/[0.02] p-2 rounded-lg border border-white/5">
                  ...
                  {item.shared_memory_refs && item.shared_memory_refs.length > 0 && (
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {item.shared_memory_refs.map((ref) => (
                        <span key={ref} className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 text-cyan-400 border border-white/5">
                          #{ref}
                        </span>
                      ))}
                    </div>
                  ...
                  {isNeedsReview && (
                    <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20 space-y-2.5">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-slate-400">Requires Founder Approval</span>
                      </div>
                      ...
                      {isReviewingThis ? (
                        <div className="space-y-2 pt-1">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded-lg text-xs font-semibold disabled:opacity-50"
                            >
                              Submit Revision Request
                            </button>
                            <button
                              onClick={() => setReviewingId(null)}
                              className="px-2.5 py-1 bg-white/5 text-slate-400 rounded-lg text-xs"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          <button
                            ...
                            className="flex-1 py-1.5 px-3 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/30 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5"
                          >
                            <CheckCircle2 className="w-3.5 h-3.5" /> Approve & Record Clean Cycle
                          </button>
                          <button
                            onClick={() => setReviewingId(item.id)}
                            className="py-1.5 px-3 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20 rounded-xl text-xs font-semibold transition-all flex items-center gap-1"
                          >
                            <RotateCcw className="w-3.5 h-3.5" /> Revise (Max 2)
                          </button>
                          <button
                            onClick={() => handleReview(item.id, "rejected", "Founder rejected output")}
                            className="py-1.5 px-3 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-xl text-xs font-semibold transition-all flex items-center gap-1"
                          >
