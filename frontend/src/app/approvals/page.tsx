...
export default function ApprovalsPage() {
  ...
  const dynamicApprovals: ApprovalDisplayItem[] = rawApprovals.map((item: any) => {
    ...
    return {
      ...
      iconBg: isCritical ? "bg-rose-50 text-rose-700" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300",
    ...
    <div className="flex flex-col gap-6 pb-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 self-start lg:self-auto">
          {/* Box 1: Total Pending */}
          <div className="bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-center">
          ...
          <div className="bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-center">
            ...
            <span className="text-xl font-extrabold text-slate-900 dark:text-slate-100 font-mono text-xs">
              Compliant
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold flex items-center gap-1.5 hover:bg-slate-50 dark:bg-slate-950 shadow-xs"
          >
      ...
      {approvalsToDisplay.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 flex flex-col items-center justify-center text-center gap-3 border border-dashed border-slate-200 dark:border-slate-700 shadow-xs">
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">All Clear &bull; No Pending Approvals</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          {approvalsToDisplay.map((approval) => (
            <div key={approval.id} className="lg:col-span-12 bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6 hover:shadow-md transition-all">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${
                      approval.priorityType === "critical" ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-700"
                    }`}>
                      • {approval.priority}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-2xl leading-relaxed">
                    <span className="font-bold text-slate-800 dark:text-slate-200">{approval.requester}</span> &bull; {approval.description}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5 self-end md:self-auto shrink-0">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold hover:bg-slate-50 dark:bg-slate-950 shadow-xs"
                >
                  Ask Revision
                </button>
                <button
                  onClick={() => handleAction(approval.id, "rejected")}
                  className="px-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-rose-200 text-rose-600 text-xs font-bold hover:bg-rose-50 shadow-xs"
                >
                  ✕ Reject
                </button>
                <button
