  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (errorDesc || errorCode) {
        const decoded = decodeURIComponent(errorDesc || errorCode || "");
        if (decoded.toLowerCase().includes("not enabled") || decoded.toLowerCase().includes("unsupported provider")) {
          setErrorMsg("Social Login provider is not enabled in your Supabase project yet. Please register below using your Work Email & Password.");
        } else {
          setErrorMsg(decoded);
        }
      }
