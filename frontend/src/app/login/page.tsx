    ...
    if (typeof window !== "undefined") {
      ...
      if (errorDesc || errorCode) {
        ...
        if (decoded.toLowerCase().includes("not enabled") || decoded.toLowerCase().includes("unsupported provider")) {
          setErrorMsg("Social Login provider is not enabled in your Supabase project yet. Please sign in with your Email & Password below, or enable providers in Supabase Dashboard (Authentication → Providers).");
        } else {
          setErrorMsg(decoded);
        }
      }
    ...
    try {
      ...
      if (error) {
        if (error.message.toLowerCase().includes("not enabled") || error.message.toLowerCase().includes("unsupported")) {
          setErrorMsg(`${provider === "github" ? "GitHub" : "Google"} OAuth is not enabled in your Supabase project. Use Email & Password below, or enable it in Supabase (Authentication → Providers).`);
        } else {
          setErrorMsg(error.message);
        }
      }
  ...
  return (
    <div className="w-full max-w-[440px] mx-auto py-12 px-4 flex flex-col items-center justify-center">
      <div className="w-full bg-white dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200/90 dark:border-slate-700/90 rounded-2xl shadow-2xl shadow-slate-200/60 p-8 sm:p-9">
        ...
        <div className="grid grid-cols-2 p-1 bg-slate-100/90 rounded-xl mb-5 text-xs font-semibold">
          <button
            type="button"
